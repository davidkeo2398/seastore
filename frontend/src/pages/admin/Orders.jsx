...
export default function OrdersPage() {
  ...
  const handleStatusChange = async (order_id, newStatus) => {
    try {
      ...
      // Cập nhật trạng thái đơn hàng trong
      setOrders((prevOrders) =>
        //ds đơn hàng hiện tại => kiển tra nếu order_id trùng với order_id của đơn hàng hiện tại thì cập nhật trạng thái mới
  ...
  const handleViewDetails = (order) => {
    setSelectedOrder(order); //lưu thông tin đơn hàng
    setIsDetailDialogOpen(true); // mở dialog chi tiết đơn hàng
  };

  //lọc tìm kiếm và trạng thái đơn hàng
  const filteredOrders = orders.filter((order) => {
    const matchesSearch =
      order.user_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      order.order_id.toLowerCase().includes(searchTerm.toLowerCase());

    // // xem xét trạng thái đơn hàng,
    // const matchesStatus =
  ...
  //     // case "shipped":
  //     //   return (
  //     //     <Badge className="text-purple-800 bg-purple-100 hover:bg-purple-200">
  //     //       Đã gửi
  //     //     </Badge>
  //     //   );
  //     case "completed":
